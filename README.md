...
A reference root project of a Horde 6 installation.

You can safely replace this root project with your own skeleton.
Nothing in Horde 6 has a hard dependency on anything shipped here.
The horde/horde-installer-plugin will generate a subset of this skeleton's directories and place autogenerated files there.
...
- web/ dir contains web readable content, installed horde apps, javascript and themes
  ...
  - web/static contains autogenerated, web-readable content like cached css. It will be lost on composer upgrades. This dir must be writeable by PHP (i.e. www-data for php-fpm) and readable by the webserver.
- vendor/ dir is managed by composer. It's where libraries and tools are installed. Must not be exposed to web.
...
- var/ contains some default locations for variable data which you might want to backup
  - var/config/ contains persistent configuration set by the administrator or automated tools. These will be symlinked to vendor/horde/$app/config/ location.
    Any custom files or modifications directly stored in vendor/horde/$app/config are lost on package upgrades.
...
```
git clone https://github.com/horde/bundle.git bundle
cd bundle
composer install
```
...
Not all horde apps have already been released as beta quality apps yet.
You must specifically allow installing alpha quality apps and libraries if you want to use them.

Adding alpha quality apps
